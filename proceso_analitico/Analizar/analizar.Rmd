...
tiempo_viaje_por_dia_tipo <- cyclistic_data %>% 
  group_by(member_casual, viajes_dia_semana = weekdays(fecha_inicio), año = year(fecha_inicio)) %>% 
...
grafico_duracion_viaje_por_tipo <- ggplot(data = tiempo_viaje_por_dia_tipo) +
  geom_point(mapping = aes(x = viajes_dia_semana, y = duracion_promedio,
                           color=viajes_dia_semana)) +
  facet_grid(member_casual~año) +
...
distancia_viaje_por_mes_tipo <- cyclistic_data %>% 
  group_by(member_casual, mes = month(fecha_inicio, label= TRUE), año = year(fecha_inicio)) %>% 
...
grafico_distancia_promedio_viajes_mensuales_por_tipo <-
  ...
  geom_col(mapping = aes(x = mes, y = distancia_promedio, fill = member_casual)) +
  facet_grid(member_casual~año) +
